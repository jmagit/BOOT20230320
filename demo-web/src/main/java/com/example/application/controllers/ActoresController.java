...
public class ActoresController {
	...
	@PostMapping("/add")
	public ModelAndView addPOST(@ModelAttribute("elemento") @Valid Actor item, BindingResult result, Locale locale) throws BadRequestException {
		...
		if (!result.hasErrors()) {
			try {
				...
				result.addError(new FieldError("elemento", "actorId", e.getMessage()));
			} catch (Exception e) {
	...
	public ModelAndView editPOST(@PathVariable Long id, @ModelAttribute("elemento") @Valid Actor item,
		...
		if (id != item.getActorId())
			result.addError(
					new FieldError("elemento", "actorId", ms.getMessage("errormsg.clave.invalida", null, locale)));
		if (result.hasErrors()) {
